...
final class Logger
{
    ...
    public function status(string $sender, string $message, bool|float|int|string ...$values): void
    {
        $this->echo("\033[47;1;30m ".$sender." \033[0m ".\sprintf($message, ...$values)."\n\n", false);
    }

    public function info(string $message, bool|float|int|string ...$values): void
    {
        $this->echo("\033[32m".\sprintf($message, ...$values)."\033[0m\n", !$this->verbose);
    }

    public function debug(string $message, bool|float|int|string ...$values): void
    {
        $this->echo("\033[34m".\sprintf($message, ...$values)."\033[0m\n");
    }

    public function error(string $message, bool|float|int|string ...$values): void
    {
