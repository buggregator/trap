...
final class Http
{
    ...
    public static function parseMultipartBody(StreamInterface $stream, string $boundary): iterable
    {
        ...
        try {
            while (false !== ($pos = StreamHelper::strpos($stream, $findBoundary))) {
                ...
                if (false === $blockEnd || $blockEnd - $pos <= 2) {
                    break;
                }
                // Parse part headers
                ...
                if ($part instanceof File) {
                    ...
                    false !== $endOfContent or throw new \RuntimeException('Missing end of content');

                    $part = $part->withValue(0 < $endOfContent ? $stream->read($endOfContent) : '');
                }
                $result[] = $part;
            }
        } catch (\Throwable $e) {
    ...
    private function parseBody(StreamClient $stream, ServerRequestInterface $request): ServerRequestInterface
    {
        ...
        return match (true) {
            'application/x-www-form-urlencoded' === $contentType => self::parseUrlEncodedBody($request),
    ...
    private function createBody(StreamClient $stream, ?int $limit): StreamInterface
    {
        ...
        foreach ($stream->getIterator() as $chunk) {
            if (null !== $limit && \strlen($chunk) + $written >= $limit) {
            ...
            if (null === $limit && ! $stream->hasData() && \str_ends_with($chunk, "\r\n\r\n")) {
